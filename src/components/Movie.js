  ...
  return (
    <div className="bg-white rounded-md shadow flex-1">
      <div className="p-5 pb-3 border-b border-zinc-200">
        <h4 className="text-xl font-bold">{movies.title} Detayları</h4>
      </div>
      ...
      <div className="px-5 py-3 border-t border-zinc-200 flex justify-end gap-2">
        <button onClick={handleRemove} type="button" className="myButton bg-red-600 hover:bg-red-500" > 
          Sil
        </button>
        <button onClick={() => dispatch(addFavorite(movie)) } className="myButton bg-blue-600 hover:bg-blue-500 ">Favorilere ekle</button>
      </div>
    </div>
  );
}
